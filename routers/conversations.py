...
async def get_allegro_threads(allegro_account_id: int, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def get_allegro_thread_messages(allegro_account_id: int, thread_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    client = await get_allegro_client(allegro_account_id, current_user.id, db)
...
async def post_allegro_thread_message(allegro_account_id: int, thread_id: str, message: MessageCreate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    client = await get_allegro_client(allegro_account_id, current_user.id, db)
...
async def get_allegro_issues(allegro_account_id: int, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def get_allegro_issue_details(allegro_account_id: int, issue_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def get_allegro_issue_messages(allegro_account_id: int, issue_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def post_allegro_issue_message(allegro_account_id: int, issue_id: str, message: MessageCreate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def get_allegro_offer_details(allegro_account_id: int, offer_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    client = await get_allegro_client(allegro_account_id, current_user.id, db)
    return await client.get_offer_details(offer_id=offer_id)

@router.get("/{allegro_account_id}/orders/{order_id}", response_model=dict)
async def get_allegro_order_details(allegro_account_id: int, order_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    client = await get_allegro_client(allegro_account_id, current_user.id, db)
